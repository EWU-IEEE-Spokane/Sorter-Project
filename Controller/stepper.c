...
uint8_t limitDebounce() {
    if (HOME == 0x00) {       // If switch activated
        delayT(1000);         // Wait for bouncing to stop
        if (HOME == 0x00) {   // If switch still activated
...
void homingMode() { // data = 0000 00 & direction & limitSw
    while((limitDebounce() == 0x00)) {          // while the mode is unchanged and the switch is unpressed
        stepOnce(DIRECTION_CCW);                                 // continuously step in the specified direction
        delayT(10000);
    }
